...
function checkPalindrome(){
    ...
    const yesOrNo = isPalindrome(inputText)? 'is a palindrome': 'is not a palindrome';


    const pTag = `<p class="${yesOrNo.toLocaleLowerCase()}"><strong><span>${inputText}</span></strong> ${yesOrNo}</p>`
    if(inputText.trim() === ''){
